...
\JCO{Yet, in the general case,}{In summary} H-BD trades overhead reduction for accuracy. 
H2 then asserts that, \JC{\emph{in practice}}, the tradeoff posed by H-BD is effective. \JC{Below, we discuss our empirical evaluation of this hypothesis.}

\JC{\paragraph{Subjects} For accuracy comparison, we selected a set of real-world applications using the following methodology
to avoid evaluators' bias:} We started from the 65 Google Play apps not chosen for the training phase. We then excluded 8 apps that do not have permission to access sensitive data and/or perform release operations (i.e., their manifest does not declare sufficient permissions out of {\tt INTERNET}, {\tt READ\_PHONE\_STATE}, {\tt SEND\_SMS}, etc), as well as 3 apps that we did not manage to install properly, resulting in 54 apps that installed successfully and exercise privacy sources and sinks.
...
%}% resizebox
\end{center}
\caption{\label{Ta:realworld}Findings due to the H-BD and T-BD \Tool\ configurations on 15/54 top-popular mobile apps (phone number abbreviated as no.; device ID as dev. ID; and location as loc.)}
\end{small}
\end{table*}
...
Moreover, H-BD has a lower number of crashes and lower runtime overhead, which \emph{confirms H2}.
...
\JC{
As indicated in \tableref{realworld}, the findings by H-BD are more complete:
it detects 27 leakages (vs. 13 for T-BD), with no false negative results and only one false positive.
