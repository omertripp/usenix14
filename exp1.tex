...
We then checked whether the difference in accuracy is statistically significant by testing the null hypothesis that the set of 48 observations from TaintDroid comes from the same Bernoulli distribution as the set of 48 observations from \Tool.
...
We found a p-value of 0.001, which rejects the null hypothesis that the observations come from the same underlying distribution and provides evidence that \Tool\ is more accurate than TaintDroid, confirming H1.
}
